...
set_sleep <- function(s){
  assert_that(!missing(s), msg = "Missing ms")
  options("LOC_SLEEP" = s)
}
...
#' 
#' @export
loc_search_titles <- function(terms, pages = 1, sleep = get_sleep()) {
  assert_that(!missing(terms), msg = "Missing terms")

  if(is.infinite(pages)) pages <- 999

  parsed_url <- parse_url(BASE_URL)
  parsed_url$path <- c("search", "titles", "results")
  opts <- list(terms = terms, format = "json")
